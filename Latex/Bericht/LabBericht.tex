...
If a $2$ cell is added, a relation is noted, namely the unique circle that describes the border of the $2$ cell. Every time a $2$ cell is added, we have to check if a circle dies. For this we check the kernel of the matrix:
 	...
 	An example in Fig. \ref{fig4}. We are given two circles in blue, and a relation in red. If we number the edges as in the first picture and select all circle orientations counterclockwise we get the following matrix:
$$A=\begin{Bmatrix}
1 & 0 & -1\\
0 & 1 & 1\\
0 & -1 & -1\\
