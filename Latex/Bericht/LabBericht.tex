...
\begin{document}
    ...
    When further analysing case 4 of the previous subsection we discovered, that there is actually more room for improvement.
    As shown in Fig.~\ref{cases} The actions performed in case 4 do not destroy any information in the homotopy.
    \input{caseanalysis}
    The homotopy equivalence is given by sending the edges $b$,$c$ and $d$ to the null-loop.
    This is a homotopy equivalence, since the contracted set $\{a,b,c\}$ is null-homotopic.\\
    However we noticed, that alot of steps can be skipped, if we respect this homotopy when we add edges already.
    ...
    Then checking for closed paths inside the graph reduces to checking whether the inserted edge is a loop and cases 2.2 and 4 get obsolete.
    ...
    This improvement actually allows us to half the already optimized computation time of persistence diagram.\\
    ...
    \begin{itemize}
        ...
        \item Ctrl+Shift+C: Contract all new edges if possible (see\ \ref{subsec:further-optimizations})
