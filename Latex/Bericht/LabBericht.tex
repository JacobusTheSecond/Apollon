...
\begin{document}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw (h) -- (b);
        \end{tikzpicture}
        \caption{\textit{A Čech-Complex is a given $\varepsilon$ with the generating dataset.}}
    \end{figure}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw[-{Latex[length=2mm,width=2mm]},color=blue] (b) -- (h);
        \end{tikzpicture}
        \caption{Beispiel der Kreisvektoren.
        In Rot der Kreis der Relation, in Blau zwei Kreise, die durch die rote Relation miteinander identifiziert werden}
        \label{fig4}
    \end{figure}
    ...
    Now to actually use the persistent homology and the distance measures on images, we need to extract features, which will be afterwards converted to 2D points.
    To do this we first considered an image, converted it to grayscale and then tried out existing detection algorithms.
    the ones tested are edge detection
