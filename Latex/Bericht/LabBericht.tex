...
\begin{document}
    ...
    In that case we contract every non-loop edge, that was not mapped using the face relation due to simplifying the underlying graph.
    That can be done as can be seen in Fig.\ \ref{figBLA} because the resulting graph is homotopy equivalent to the previous one.%TODO Image ref
    But why not use the homotopy equivalence already when adding any edge?
    ...
    Instead of the already few $\sim 30$ seconds for 1000 nodes we only need $\sim 15$ seconds for the whole persistent homology computation.
